...
function displayBusRoute(bus) {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      // Use geocodeLocation function to get geocoded locations for start and end stations
      Promise.all([
        ...
        directionsService.route({
          ...
          const busRoute = [startLocation, ...waypoints.map(waypoint => waypoint.location), endLocation]; // Include all points (current location, waypoints, start & end stations)

          busRoute.forEach((waypoint) => {
            new google.maps.Marker({
              position: waypoint,//waypoint.location,
              map: map,
              title: 'Waypoint',
              icon: {
          ...
          distanceMatrixService.getDistanceMatrix(
            {
              ...
              destinations: busRoute,
              travelMode: google.maps.TravelMode.WALKING, // passenger needs to walk to the bus stop
            },
            (response, status) => {
              if (status === google.maps.DistanceMatrixStatus.OK) {
                ...
                const closestStationLocation = busRoute[closestStationIndex + 1]; // +1 to account for current location
                
                console.log(closestStationLocation)
                //Position the bus stop marker at the closest station

                var busStopMarkerIcon = {
                ...
                new google.maps.Marker({
                  position: closestStationLocation,
                  map: map,
                  title: 'Bus!',
